...
class AdministratorScreen(Screen):
    ...
    def __init__(self, **kwargs):
        ...
        layout.add_widget(Button(text='Импорт расписания',background_color=[0, 1.5, 3, 1],on_press=self.BUTTON_ImportTimesheet))
        layout.add_widget(Button(text='Редактирование данных',background_color=[0, 1.5, 3, 1],on_press=self.BUTTON_EditData))
        layout.add_widget(Button(text='Уведомления', on_press = self.BUTTON_Message))
        layout.add_widget(Button(text='Управление учетными записями',background_color=[0, 1.5, 3, 1],on_press = self.BUTTON_users))
        ...
        bottommenu.add_widget(Button(text='Выход', on_press=self.BUTTON_exit,background_color=[0, 1.5, 3, 1], size_hint=(.5, 1)))
